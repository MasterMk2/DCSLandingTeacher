...
- Web UI での閲覧
  ...
  - トップダウン軌跡ビュー、時系列チャート（偏差・速度・AOA・降下率・荷重倍数 G / バンク角）
...
米海軍式の LSO グレーディングに基づき、FLOLS 想定のグライドスロープ（3.5°、3 番ワイヤー基準）と
アングルドデッキ（艦首方位から左舷へ 9.14°）の中心線からの偏差から
**OK / OK- / (OK) / _NO_GRADE_ / CUT** を自動付与します。
加えて ARCON / AOC / AOS / FAST / SLOW / HIGH / LOW / OFFLINE / BOLTER 等の
ファクターを検出し、根拠データとともに記録します。

偏差は **艦と一緒に動く座標** で、甲板からの高さで測ります（艦は 15 m/s 前後で
進むので、接地時刻の艦位置で止めると手前ほど「低い」と出ます。Tacview の AGL は
海面基準なので使いません）。艦の幾何は MOOSE AIRBOSS の値（DCS 本体のデータ
ファイルを出典とするもの）を `config/carriers.yaml` に持っています。

**Case I パターンもキスオフから記録します。** 着艦の記録は陸上と同じく接地前
300 秒 / 8 nm まで遡るので、イニシャル → ブレイク（キスオフ）→ ダウンウィンド →
180°ターン（90 / ウェイク）→ グルーブ → 着艦 がすべて入ります。艦の座標系
（艦首方向が上）で、キスオフの位置と G・バンク、アビーム距離と高度、90 と
ウェイク通過の高度、グルーブ時間（15〜19 秒、外れれば NESA / LIG）を測り、
平面図と講評に出します。パターンは **測定と講評のみ** で、グレードには効きません。

艦載機は硬めに降りるのが前提なので、着艦の接地降下率は減点しません。代わりに
ランプ直前でフレアした（沈下を止めた）ことを指摘します。
...
接地降下率の許容幅は機体クラス別です。F/A-18・F-14・Su-33 などの **艦載機** は
陸上でもフレアせずに降ろす（FCLP）ことがあり、脚もそれを前提に作られているので、
戦闘機より硬い接地まで「良好」とする専用の幅（〜850 fpm）で判定します。
