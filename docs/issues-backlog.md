...
> **対応状況（2026-08-25 実サーバ検証で確定）**: D-1 / D-2 / D-4 / D-5 は実装・
> テスト済み。D-2 は実際の DCS サーバに接続してストリームをダンプし、
> `TAS`/`CAS`/`IAS` が単位以前に一切出力されないことを確認した上で
> `TrackIngestor._ground_speed_ms()`（位置微分フォールバック）を実装した。
...
- **状態**: 対応済み（2026-08-25）— 稼働中の DCS サーバに直接接続してハンドシェイク後の
  生ストリームをダンプし、実機オブジェクトの行 (`Type=Air+FixedWing` 等) を確認したところ
  `TAS`/`CAS`/`IAS` はいずれも一切出力されていなかった（単位の問題以前に、プロパティ自体が
  DCS のエクスポータから送信されない）。よって単位変換では解決できず、
  `TrackIngestor._ground_speed_ms()` として位置微分（haversine 距離 / dt）による
  対地速度フォールバックを実装。回帰テスト
  (`test_ingest_derives_ground_speed_when_acmi_omits_speed_properties`) 込み。
- **種別**: 不具合 / ラベル: `bug`, `grading`
- タッチダウン速度・進入速度比（FAST/SLOW 判定）が実データで全件失敗していた
- 原因: DCS Tacview エクスポータが `TAS`/`CAS`/`IAS` プロパティ自体を一切出力しないため
  `AcmiObject.speed` が常に `None` になっていた（単位変換の問題ではなかった）
- 対応: `speed` が欠落している場合、当該機体の直前サンプルとの
  haversine 距離 / dt から対地速度を導出し `TrackSample.speed` に適用
  （最初のサンプルや `dt <= 0.05s` の場合は `None` のまま）
- 受け入れ条件: 実データの着陸で `touchdown_speed_ms` が妥当な値になり、
  FAST/SLOW が判定される — 本番の `ffs-dcs-landing-teacher` へデプロイ後、
  新規着陸で確認予定
...
- **追補（2026-08-26）**: 上記の対応から 2 箇所漏れており、ユーザーから
  「表現にメートルが生き残っている」と指摘されて修正した。
  グラフや GCA スコープは ft なのに評価画面だけメートルという状態だった。
  1. `land_grader._build_comment()` が生成する日本語の講評文
     （「平均 16.9 m」「最大 28.0 m」）。バックエンド生成の文字列なので
     フロントの変換層を通らない。文面に出す値だけ ft に換算し、
     しきい値比較は SI のまま残した。
  2. 「根拠データ」「評価メトリクス」表が payload の生キーをそのまま出しており
     （`max_abs_deviation_m: 28.01`）、単位も未変換だった。`format.ts` の
     `formatMetric()` でキー接尾辞（`_m` / `_ms` / `_fpm` / `_deg` / `_s`）から
     単位を判定して換算し、値と矛盾しないよう接尾辞を落として表示する。
     `_ms` は降下率（fpm）と対気速度（kt）で読みが違うのでキー名で分岐する。
